...
import { ActivityIndicator, Button, Platform, Alert } from "react-native";
...
const App = () => {
  const { userId, userLoading, getLocalUser } = useContext(UserContext);
    ...
    if (userId !== null && !responseListener.current) {
      ...
      responseListener.current =
        Notifications.addNotificationResponseReceivedListener((response) => {
          ...
          if (response.notification.request.content.data.navigateTo) {
        ...
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ focused, color, size }) => {
            let iconName:
              ...
              | "list-circle-outline"
              | "person-circle"
              | "person-circle-outline" = "list-circle";
